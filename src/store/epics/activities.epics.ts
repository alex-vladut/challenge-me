...
const createActivity = (actions$: Observable<ActionWithPayload<any>>) =>
  actions$.pipe(
    ...
    switchMap(({ payload }) =>
      from(
        API.graphql(
          graphqlOperation(mutations.createActivity, { input: payload })
        )
      )
    ),
      ...
      of(
        CreateFail.create(
          response.errors[0].message ||
            "There was an error while creating your activity. Please try again."
        )
      )
    )
  );
...
const fetchActivities = (actions$: Observable<Action>) =>
  actions$.pipe(
    ofType(Fetch.type),
    switchMap(() =>
      from(API.graphql(graphqlOperation(listActivitys, { limit: 10 })))
    ),
    ...
    catchError(response =>
      of(
        FetchFail.create(
          response.errors[0].message ||
            "Sorry, there was an error while loading the activities."
        )
      )
    )
  );
